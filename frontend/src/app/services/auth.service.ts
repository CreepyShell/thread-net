...
export class AuthenticationService {
    ...
    public register(user: UserRegisterDto) {
        return this._handleAuthResponse(
            this.httpService.postFullRequest<AuthUser>(
    ...
    public revokeRefreshToken() {
        return this.httpService.postFullRequest<AccessTokenDto>(
            `${this.routePrefix}/token/revoke`,
            {
    ...
    public refreshTokens() {
        return this.httpService
            .postFullRequest<AccessTokenDto>(
                `${this.routePrefix}/token/refresh`,
                {
    ...
    private _setTokens(tokens: AccessTokenDto) {
        if (tokens && tokens.accessToken && tokens.refreshToken) {
            localStorage.setItem(
                "accessToken",
                JSON.stringify(tokens.accessToken.token)
            );
            localStorage.setItem(
                "refreshToken",
