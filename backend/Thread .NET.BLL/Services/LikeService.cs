    ...
    public sealed class LikeService : BaseService
    {
        ...
        public async Task LikePost(NewReactionDTO reaction)
        {
            ...
            if (likes.Any())
            {
                ...
                if (likes.First().IsLike != reaction.IsLike)
                {
                    _context.PostReactions.Where(x => x.UserId == reaction.UserId && x.PostId == reaction.EntityId).First().IsLike = reaction.IsLike;
                    _context.Update(likes.First());
